...
> 本项目以轻量，易部署为目标
>
> 静态网页展示风格格式参考 [newsnow](https://github.com/ourongxing/newsnow) 仓库修改
> 项目 fork 自 [TrendRadar](https://github.com/sansan0/TrendRadar)
...
项目内置 **29 个独立数据源**爬虫，默认启用 24 个平台，覆盖综合社交、新闻资讯、科技、财经、国际五大类别：
...
> ⬜ 标记的平台已内置爬虫实现，可在 `config.yaml` 的 `platforms.sources` 中手动启用
...
支持 RSS 2.0 / Atom / JSON Feed 订阅源抓取，按关键词分组统计（与热榜格式一致）。默认配置 **16 个 RSS 源**：
...
- **简单配置**：直接在 `config.yaml` 的 `rss.feeds` 中添加任意 RSS/Atom 订阅地址
- **合并推送**：热榜和 RSS 合并为一条消息推送
- **新鲜度过滤**：支持全局和单源级别的文章年龄过滤，默认只推送 3 天内的文章
- **自定义扩展**：可自行添加任何标准 RSS/Atom/JSON Feed 订阅源
...
### 5️⃣ 第五步：远程云存储配置
   ...
   > 💡 已配置 Keepalive workflow，GitHub Actions 会自动保持活跃运行，无需手动操作。
